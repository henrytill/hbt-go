...
- **A creation time can be absent, and an absent one contributes nothing.** `CreatedAt` carries a `Valid` flag like `LastVisitedAt`, so an anchor with no `ADD_DATE` parses to an entity with no creation time rather than one created now (#87) or at the epoch. `CreatedAt.Merge` makes absence the identity: a dated mention merged with an undated one keeps its own instant, and neither operand pushes an absence into the history. This is henrytill/hbt-data#37, and it is why the merge cannot be a minimum over the zero value, which sorts below every real instant and would win. Two undated mentions stay undated.
- **Absence has a wire representation**, which is what makes the rule survive a round-trip: `entityRepr.CreatedAt` is a `*int64` with `omitempty`, so `createdAt` is omitted when absent as `lastVisitedAt` and `shared` already are. Before that, an undated entity serialized as `createdAt: 0` and decoded back as one genuinely created on 1970-01-01, so the same collection merged differently depending on whether it had passed through YAML. A real creation time of `0` still serializes, and still exports `ADD_DATE="0"` -- the template tests a pointer for presence, where testing the `int64` for truth dropped it (#74).
...
- **The identical-entity guard in `absorb` is still load-bearing**, and so are its counterparts in the other three -- none of them is inert. What differs is only how far the protected shape reaches: exported fields here and in hbt-hs, so any package or module can write it; an abstract type in hbt-ocaml whose `make` still accepts a history it deliberately does not normalize; and private fields in hbt-rs, where only its own module can, which its own tests do. `TestUpsertIdenticalEntityIsNoOp` builds it here, and removing the guard fails that test and nothing else.
